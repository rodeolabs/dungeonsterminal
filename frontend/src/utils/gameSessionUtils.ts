...
export const gameSessionUtils = {
  ...
  createInitialGameState: (character: Character): GameState => {
    return {
      ...
      spellSlots: Object.fromEntries(
        Object.entries(character.spellSlots).map(([level, slots]) => [level, slots.current])
      ),
  ...
  getAvailableSpellSlots: (character: Character): number => {
    return Object.values(character.spellSlots).reduce(
      (total, slots) => total + slots.current,
      0
    );
  },
