...
describe('useSocketConnection', () => {
    ...
    describe('initialization', () => {
        ...
        it('should create socket with default configuration', () => {
            ...
            expect(mockIo).toHaveBeenCalledWith(socketConnectionTestUtils.DEFAULT_CONFIG.serverUrl, expect.objectContaining({
        ...
        it('should create socket with custom configuration', () => {
            ...
            expect(mockIo).toHaveBeenCalledWith(customConfig.serverUrl, expect.objectContaining({
    ...
    describe('connection quality monitoring', () => {
        ...
        it('should monitor connection quality with heartbeat', () => {
            const { result } = renderHook(() => useSocketConnection({ enableHeartbeat: true, heartbeatInterval: 1000 }));
            // Connect first
            act(() => {
                const connectHandler = mockSocket.on.mock.calls.find(call => call[0] === 'connect')?.[1];
                connectHandler?.();
    ...
    describe('development utilities', () => {
        ...
        it('should expose debug utilities in development', () => {
            renderHook(() => useSocketConnection());
            expect(window.socketDebug).toBeDefined();
            expect(window.socketDebug.getConnectionState).toBeInstanceOf(Function);
            expect(window.socketDebug.forceReconnect).toBeInstanceOf(Function);
        });
        it('should not expose debug utilities in production', () => {
            process.env.NODE_ENV = 'production';
            renderHook(() => useSocketConnection());
            expect(window.socketDebug).toBeUndefined();
        });
    });
});
//# sourceMappingURL=useSocketConnection.test.js.map